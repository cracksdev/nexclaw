...
const NEXCLAW_REPO_URL = 'https://github.com/YOUR_ORG/nexclaw';
...
async function launchNexClaw() {
  const configured = vscode.workspace.getConfiguration('nexclaw');
  const launchCommand = configured.get('launchCommand', 'nexclaw');
  const terminalName = configured.get('terminalName', 'NexClaw');
  ...
  if (!installed) {
    const action = await vscode.window.showErrorMessage(
      `NexClaw command not found: ${executable}. Install it with: npm install -g @gitlawb/nexclaw`,
      'Open Repository'
    );

    if (action === 'Open Repository') {
      await vscode.env.openExternal(vscode.Uri.parse(NEXCLAW_REPO_URL));
    }
...
class NexClawControlCenterProvider {
...
<body>
  <div class="panel">
    <div class="topbar">
      <span>nexclaw control center</span>
    ...
    <div class="content">
      ...
      <div class="terminal-box">
        <div class="terminal-row"><span class="prompt">$</span> nexclaw --status</div>
      ...
      <div class="actions">
        <button class="btn primary" id="launch">Launch NexClaw</button>
        <button class="btn" id="docs">Open Repository</button>
        <button class="btn" id="commands">Open Command Palette</button>
      </div>

      <div class="hint">
        Quick trigger: use <code>${status.shortcut}</code> and run NexClaw commands from anywhere.
      </div>
    </div>
  </div>
...
function activate(context) {
  const startCommand = vscode.commands.registerCommand('nexclaw.start', async () => {
    await launchNexClaw();
  });

  const openDocsCommand = vscode.commands.registerCommand('nexclaw.openDocs', async () => {
    await vscode.env.openExternal(vscode.Uri.parse(NEXCLAW_REPO_URL));
  });

  const openUiCommand = vscode.commands.registerCommand('nexclaw.openControlCenter', async () => {
    await vscode.commands.executeCommand('workbench.view.extension.nexclaw');
  });

  const provider = new NexClawControlCenterProvider();
  const providerDisposable = vscode.window.registerWebviewViewProvider('nexclaw.controlCenter', provider);
