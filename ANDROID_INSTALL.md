# NexClaw on Android (Termux)

A complete guide to running NexClaw on Android using Termux + proot Ubuntu.
...
NexClaw requires [Bun](https://bun.sh) to build, and Bun does not support Android natively. The workaround is running a real Ubuntu environment inside Termux via `proot-distro`, where Bun's Linux binary works correctly.
...
### Step 3 — Clone NexClaw

```bash
git clone https://github.com/YOUR_ORG/nexclaw.git
cd nexclaw
npm install
npm link
```
...
### Step 6 — Build NexClaw

```bash
cd /data/data/com.termux/files/home/nexclaw
bun run build
```

You should see:
```
✓ Built nexclaw v0.1.6 → dist/cli.mjs
```
...
Both were tested and fail due to NexClaw's large system prompt (~50K tokens):
...
- Run `bun run build` again only if you pull updates to the NexClaw repo.
