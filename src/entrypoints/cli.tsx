...
async function main(): Promise<void> {
  ...
  // Fast-path for --gui-mode: headless JSON-RPC mode for desktop GUI
  if (args.includes('--gui-mode') || process.env.OPENCLAUDE_GUI_MODE === '1') {
    const { guiEmitter } = await import('../gui-adapter/guiEventEmitter.js');
    const { startInputHandler } = await import('../gui-adapter/guiInputHandler.js');
    const { emitEvent } = await import('../gui-adapter/protocol.js');

    guiEmitter.emitReady();

    startInputHandler((command) => {
      switch (command.type) {
        case 'send_message':
          emitEvent({ event: 'ack', id: command.id, status: 'received' });
          break;
        case 'cancel':
          emitEvent({ event: 'ack', type: 'cancel', status: 'received' });
          break;
        case 'tool_response':
          emitEvent({ event: 'ack', id: command.id, type: 'tool_response', status: 'received' });
          break;
        case 'set_config':
          emitEvent({ event: 'config_updated', key: command.key, value: command.value });
          break;
      }
    });
    return;
