/**
 * NexClaw build script — bundles the TypeScript source into a single
...
const result = await Bun.build({
  ...
  define: {
    ...
    'MACRO.PACKAGE_URL': JSON.stringify('@gitlawb/nexclaw'),
...
`,
        ...
        for (const mod of [
          ...
          'code-excerpt',
          'stack-utils',
          // Cloud provider SDKs — stubbed so they don't need to ship with the
          // packaged Electron app. They throw only if actually invoked (Bedrock/Vertex/Azure).
          // Stub the high-level wrappers first so their internal AWS/Google imports
          // are never processed by the bundler (Bun doesn't apply onResolve hooks
          // to imports found inside already-bundled node_modules packages).
...
export const resourceFromAttributes = () => ({ merge: () => ({ merge: () => ({ merge: () => ({}) }) }), attributes: {} });
...
export const trace = { getTracer: () => ({ startSpan: () => ({ end: noop, setAttribute: noop, setStatus: noop, recordException: noop }) }), setGlobalTracerProvider: noop };
export const context = { active: noop, with: (_, fn) => fn() };
export const logs = { setGlobalLoggerProvider: noop, getLogger: () => ({ emit: noop }), getLoggerProvider: () => ({}) };
export const diag = { setLogger: noop, debug: noop, info: noop, warn: noop, error: noop };
export const DiagLogLevel = { ALL: 0, VERBOSE: 1, DEBUG: 2, INFO: 3, WARN: 4, ERROR: 5, NONE: 6 };
...
export const ConsoleLogRecordExporter = noopClass;
export const ConsoleMetricExporter = noopClass;
export const ConsoleSpanExporter = noopClass;
export const envDetector = { detect: () => ({ attributes: {} }) };
export const hostDetector = { detect: () => ({ attributes: {} }) };
export const osDetector = { detect: () => ({ attributes: {} }) };
`,
            loader: 'js',
          }),
        )

        // @opentelemetry/* — wildcard stub (kept external before, but not available
        // in the packaged Electron app; the Proxy stub handles all named exports)
...
console.log(`✓ Built nexclaw v${version} → dist/cli.mjs`)
